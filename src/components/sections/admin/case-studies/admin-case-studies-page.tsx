...
import { AdminCaseStudyForm, type CaseStudyFormData } from "./admin-case-study-form";
...
type Props = {
  initialItems: CaseStudyRow[];
};

const STATUS_LABELS = { DRAFT: "Taslak", PUBLISHED: "Yayında" } as const;

export function AdminCaseStudiesPageSections({ initialItems }: Props) {
  const [items, setItems] = useState<CaseStudyRow[]>(initialItems);
  const [editing, setEditing] = useState<CaseStudyRow | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [deleting, setDeleting] = useState<number | null>(null);

  async function handleDelete(id: number) {
    if (!confirm("Bu vaka çalışmasını silmek istediğinize emin misiniz?")) return;
    setDeleting(id);
    try {
  ...
  async function refreshItems() {
    try {
      ...
      if (res.ok) {
        const body = await res.json() as { items: CaseStudyRow[] };
        setItems(body.items.map((r) => ({ ...r, status: r.status as "DRAFT" | "PUBLISHED" })));
      }
    } catch {}
  }

  function openEdit(item: CaseStudyRow) {
    setEditing(item);
    setShowForm(true);
  ...
  const formInitial: Partial<CaseStudyFormData> | undefined = editing
    ? {
        slug: editing.slug,
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[color:var(--primary)]">Vaka Çalışmaları</h1>
          <p className="mt-1 text-sm text-[color:var(--app-muted)]">Portfolio detay sayfalarının içeriklerini yönetin.</p>
        </div>
        ...
        <div className="overflow-hidden rounded-xl border border-[color:var(--app-border)]/30">
          <table className="w-full text-sm">
            <thead className="bg-[color:var(--surface-container-low)]">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[color:var(--app-muted)]">Müşteri</th>
                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[color:var(--app-muted)]">Slug</th>
                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[color:var(--app-muted)]">Sektör</th>
            ...
            <tbody className="divide-y divide-[color:var(--app-border)]/20 bg-[color:var(--app-card)]">
              {items.map((item) => (
                <tr className="hover:bg-[color:var(--surface-container-low)]" key={item.id}>
                  <td className="px-4 py-3 font-medium text-[color:var(--primary)]">{item.client}</td>
                  <td className="px-4 py-3 font-mono text-xs text-[color:var(--app-muted)]">{item.slug}</td>
                  <td className="px-4 py-3 text-xs text-[color:var(--app-muted)]">{item.sector}</td>
